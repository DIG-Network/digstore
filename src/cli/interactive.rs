...
pub fn ask_recreate_store(archive_path: &Path, store_id_hex: &str, auto_yes: bool) -> Result<bool> {
    if auto_yes {
        println!("{}", "Store not found - auto-recreating (--yes flag)".green());
        return Ok(true);
    }
    
    println!();
    println!("{}", "Store not found!".red().bold());
    println!("  Archive file: {}", archive_path.display().to_string().yellow());
...
pub fn ask_overwrite_digstore(digstore_path: &Path, auto_yes: bool) -> Result<bool> {
    if auto_yes {
        println!("{}", "Repository file exists - auto-overwriting (--yes flag)".green());
        return Ok(true);
    }
    ...
    println!();
    println!("{}", "Overwriting will create a new store with a different ID.".red());
    println!("{}", "Any existing data in the old store will become inaccessible.".red());
    println!();
...
pub fn handle_missing_store(archive_path: &Path, store_id_hex: &str, project_path: &Path, auto_yes: bool) -> Result<Store> {
    ...
    if ask_recreate_store(archive_path, store_id_hex, auto_yes)? {
        ...
        println!("  1. {} Check if you're in the right directory", "Location:".green());
        println!("     {}", "cd /path/to/your/project".cyan());
        println!();
        println!("  2. {} Restore the missing store file", "Recovery:".green());
        println!("     {}", "The store may have been moved or deleted".cyan());
        println!();
        
        Err(anyhow::anyhow!("Store not found and user declined recreation"))
    }
}
