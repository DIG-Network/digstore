...
pub fn execute(json: bool, detailed: bool, performance: bool, security: bool) -> Result<()> {
    let args = StatsArgs { json, detailed, performance, security };
...
fn calculate_repository_stats(store: &Store) -> Result<RepositoryStats> {
    ...
    if store.archive.has_layer(&layer_zero_hash) {
        ...
        if let Some(root_history) = metadata.get("root_history").and_then(|v| v.as_array()) {
            ...
            if let Some(latest) = root_history.last() {
                current_generation = latest.get("generation").and_then(|g| g.as_u64()).unwrap_or(0);
            }
        }
    }
    ...
    let growth_metrics = GrowthMetrics {
        average_commit_size: if total_commits > 0 { total_storage / total_commits as u64 } else { 0 },
    ...
    let total_efficiency = deduplication_ratio + compression_ratio - (deduplication_ratio * compression_ratio);
...
fn show_stats_human(stats: &RepositoryStats, args: &StatsArgs) -> Result<()> {
    ...
    if args.detailed {
        println!("\n{}", "Growth Metrics:".bold());
        println!("  Average Commit Size: {}", format_bytes(stats.growth_metrics.average_commit_size));
        println!("  Growth Rate: {:.2} MB/day", stats.growth_metrics.growth_rate_mb_per_day);
        println!("  Commit Frequency: {:.1} commits/day", stats.growth_metrics.commit_frequency_per_day);
        println!("  File Growth: {:.1} files/day", stats.growth_metrics.file_growth_per_day);
        
        println!("\n{}", "Storage Efficiency:".bold());
        println!("  Deduplication: {:.1}% space saved", stats.storage_efficiency.deduplication_percentage);
        println!("  Compression: {:.1}% space saved", stats.storage_efficiency.compression_percentage);
        println!("  Total Efficiency: {:.1}% space saved", stats.storage_efficiency.total_efficiency_percentage);
        println!("  Raw Data Size: {}", format_bytes(stats.storage_efficiency.raw_data_size));
        println!("  Stored Size: {}", format_bytes(stats.storage_efficiency.stored_size));
    }
    
    if args.performance {
        println!("\n{}", "Performance Metrics:".bold());
        println!("  Average Chunk Size: {}", format_bytes(stats.performance_metrics.average_chunk_size));
        println!("  Chunking Efficiency: {:.1}%", stats.performance_metrics.chunking_efficiency);
        println!("  Merkle Tree Depth: {} levels", stats.performance_metrics.merkle_tree_depth);
        println!("  Proof Generation: {:.1}ms average", stats.performance_metrics.proof_generation_time_ms);
    }
    
    if args.security {
        println!("\n{}", "Security Metrics:".bold());
        println!("  Scrambling: {:.1}% of data protected", stats.security_metrics.scrambling_coverage);
        println!("  URN Access Control: {}", if stats.security_metrics.urn_access_control { "Active" } else { "Disabled" });
        println!("  Legacy Access: {}", if stats.security_metrics.legacy_access_disabled { "Disabled" } else { "Enabled" });
        println!("  Security Overhead: {:.1}%", stats.security_metrics.security_overhead_percentage);
    }
