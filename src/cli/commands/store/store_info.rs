...
fn show_store_info_human(store: &Store, args: &StoreInfoArgs) -> Result<()> {
    ...
    if args.paths {
        ...
        println!(
            "  • Global Store: {}",
            store.global_path().display().to_string().cyan()
        );
...
fn show_store_info_json(store: &Store, args: &StoreInfoArgs) -> Result<()> {
    ...
    if store.archive.has_layer(&layer_zero_hash) {
        ...
        store_info["digstore_version"] = metadata
            .get("digstore_version")
            .cloned()
            .unwrap_or(json!(null));
        store_info["format_version"] = metadata
            .get("format_version")
            .cloned()
            .unwrap_or(json!(null));
        store_info["protocol_version"] = metadata
