...
fn show_version_info() -> Result<()> {
            ...
            match vm.list_versions() {
                Ok(versions) => {
                    if !versions.is_empty() {
                        println!(
                            "  {} {}",
                            "Installed Versions:".bright_white(),
                            versions.len()
                        );
    ...
    println!(
        "  {} - Show this version information",
        "digstore version".green()
    );
    println!(
        "  {} - List all installed versions",
        "digstore version list".green()
    );
    println!(
        "  {} - Install current version with version manager",
        "digstore version install".green()
    );
    println!(
        "  {} - Install currently running binary",
        "digstore version install-current".green()
    );
    println!(
        "  {} - Install from MSI file",
        "digstore version install-msi <path>".green()
    );
    println!(
        "  {} - Install specific version from GitHub",
        "digstore version install-version <version>".green()
    );
    println!(
        "  {} - List system-installed versions",
        "digstore version list-system".green()
    );
    println!(
        "  {} - Set active version",
        "digstore version set <version>".green()
    );
    println!(
        "  {} - Update PATH for version",
        "digstore version update-path <version>".green()
    );
    println!(
        "  {} - Remove a version",
        "digstore version remove <version>".green()
    );
    println!(
        "  {} - Fix PATH ordering",
        "digstore version fix-path".green()
    );
    println!(
        "  {} - Auto-fix PATH ordering",
        "digstore version fix-path-auto".green()
    );
...
fn list_versions() -> Result<()> {
    ...
    if versions.is_empty() {
        println!(
            "{}",
            "No versions installed through version manager".yellow()
        );
        println!(
            "Run {} to install the current version",
            "digstore version install".green()
        );
        return Ok(());
    }
...
fn install_current_version() -> Result<()> {
    ...
    if !current_dir.join("Cargo.toml").exists() || !current_dir.join("src").join("main.rs").exists()
    {
...
fn install_specific_version(version: &str) -> Result<()> {
    ...
    println!(
        "{}",
        "✓ Version installation completed successfully!"
            .green()
            .bold()
    );
...
fn set_active_version(version: &str) -> Result<()> {
    ...
    println!(
        "{}",
        "✓ Active version updated successfully!".green().bold()
    );
...
fn fix_path_ordering() -> Result<()> {
    ...
    let version_managed_index = digstore_locations
        .iter()
    ...
    match version_managed_index {
        Some(0) => {
            println!();
            println!(
                "  {} Version-managed digstore is already first in PATH",
                "✓".green()
            );
        ...
        Some(index) => {
            println!();
            println!(
                "  {} Version-managed digstore found at position {}",
                "!".yellow(),
                index
            );
            println!(
                "  {} Earlier installations are taking precedence",
                "!".yellow()
            );
            println!();
            ...
            println!();
            println!(
                "  2. {} Move version-managed directory to front of PATH:",
                "Option".cyan()
            );
            println!("     setx PATH \"{};%PATH%\"", bin_dir.display());
        },
        None => {
            println!();
            println!(
                "  {} Version-managed digstore not found in PATH",
                "!".yellow()
            );
...
fn fix_path_ordering_automatically() -> Result<()> {
    println!("{}", "Automatically fixing PATH ordering...".bright_blue());
    ...
    if output.status.success() {
            ...
            .output()
        {
            ...
            _ => {
                println!(
                    "  {} PATH updated, but may need terminal restart",
                    "!".yellow()
                );
                println!(
                    "  {} Run: {}",
                    "→".cyan(),
                    "digstore --version".bright_green()
                );
            },
        }
    } else {
