...
fn analyze_specific_layer(
    ...
    let file_size = if let Some(entry) = store
        .archive
        .list_layers()
        .iter()
...
fn show_layer_human(
    ...
    layer: &crate::storage::layer::Layer,
    args: &LayersArgs,
) -> Result<()> {
    ...
    println!(
        "{}: {}",
        "Parent Hash".bold(),
        layer_info.parent_hash.cyan()
    );
    println!(
        "{}: {}",
        "Timestamp".bold(),
        format_timestamp(layer_info.timestamp)
    );
    ...
    if args.size {
        ...
        println!(
            "  Uncompressed Size: {}",
            format_bytes(layer_info.total_file_size)
        );
        if layer_info.total_file_size > 0 {
            let compression_ratio =
                1.0 - (layer_info.file_size as f64 / layer_info.total_file_size as f64);
    ...
    if layer_info.chunks_count > 0 {
        println!(
            "  Average Chunk Size: {}",
            format_bytes(layer_info.total_file_size / layer_info.chunks_count as u64)
        );
    }
    ...
    if args.chunks && !layer.chunks.is_empty() {
        ...
        println!(
            "    < 1KB: {} chunks ({:.1}%)",
            size_buckets[0],
            size_buckets[0] as f64 / layer.chunks.len() as f64 * 100.0
        );
        println!(
            "    1KB-32KB: {} chunks ({:.1}%)",
            size_buckets[1],
            size_buckets[1] as f64 / layer.chunks.len() as f64 * 100.0
        );
        println!(
            "    32KB-1MB: {} chunks ({:.1}%)",
            size_buckets[2],
            size_buckets[2] as f64 / layer.chunks.len() as f64 * 100.0
        );
        println!(
            "    > 1MB: {} chunks ({:.1}%)",
            size_buckets[3],
            size_buckets[3] as f64 / layer.chunks.len() as f64 * 100.0
        );
    }
