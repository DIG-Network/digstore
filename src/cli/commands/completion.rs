...
pub fn execute(shell: Shell) -> Result<()> {
    ...
    match shell {
        Shell::Bash => {
            ...
            println!("  {}", "digstore completion bash > ~/.local/share/bash-completion/completions/digstore".cyan());
        }
        Shell::Zsh => {
            println!("  Add the following to your ~/.zshrc:");
            println!("  {}", "eval \"$(digstore completion zsh)\"".cyan());
            println!("\n  Or save to a file in your fpath:");
            println!("  {}", "digstore completion zsh > ~/.local/share/zsh/site-functions/_digstore".cyan());
        }
        Shell::Fish => {
            println!("  Save the completion script:");
            println!("  {}", "digstore completion fish > ~/.config/fish/completions/digstore.fish".cyan());
        }
        Shell::PowerShell => {
            println!("  Add the following to your PowerShell profile:");
            println!("  {}", "Invoke-Expression (& digstore completion powershell)".cyan());
            println!("\n  Or save to a file and import it:");
            println!("  {}", "digstore completion powershell > $PROFILE\\..\\Modules\\digstore\\digstore.psm1".cyan());
        }
        Shell::Elvish => {
            println!("  Save the completion script:");
            println!("  {}", "digstore completion elvish > ~/.config/elvish/completions/digstore.elv".cyan());
        }
        _ => {
            println!("  Please refer to your shell's documentation for installing completion scripts.");
        }
    }
...
mod tests {
    ...
    fn test_completion_generation() {
        ...
        for shell in shells {
            ...
            assert!(!output.is_empty(), "No completion script generated for {:?}", shell);
        }
    }
}
