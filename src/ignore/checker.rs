...
impl IgnoreChecker {
        ...
        for parser in applicable_parsers {
            if parser.is_ignored(&absolute_path, is_dir) {
                ...
                if let Some(pattern_info) =
                    self.find_matching_pattern(parser, &absolute_path, is_dir)
                {
                    match pattern_info.1 {
                        // pattern_type
    ...
    fn find_matching_pattern(
        &self,
        parser: &DigignoreParser,
        file_path: &Path,
        is_dir: bool,
    ) -> Option<(String, crate::ignore::parser::PatternType)> {
    ...
    fn pattern_matches(
        &self,
        pattern: &crate::ignore::parser::CompiledPattern,
        file_path: &Path,
        is_dir: bool,
    ) -> bool {
    ...
    pub fn stats(&self) -> (usize, usize) {
        ...
        let total_patterns: usize = self.parsers.values().map(|p| p.patterns().len()).sum();
...
mod tests {
    ...
    fn test_hierarchical_ignore() -> Result<()> {
        ...
        assert!(matches!(
            checker.is_ignored(Path::new("test.tmp")),
        ...
        assert!(matches!(
            result,
            IgnoreResult::IncludedByNegation(_) | IgnoreResult::Included
        ));

        // Nested patterns should work
        assert!(matches!(
            checker.is_ignored(Path::new("nested/app.log")),
            IgnoreResult::Ignored(_)
        ));
        ...
        assert!(matches!(
            checker.is_ignored(Path::new("nested/temp.tmp")),
            IgnoreResult::Ignored(_)
        ));
    ...
    fn test_no_ignore_files() -> Result<()> {
        ...
        assert_eq!(
            checker.is_ignored(Path::new("any_file.txt")),
            IgnoreResult::Included
        );
