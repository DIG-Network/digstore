...
impl StoreAccessControl for Store {
    ...
    fn get_file_secure(&self, urn: &Urn) -> SecurityResult<Vec<u8>> {
        ...
        let file_data = self.get_file_at(
    ...
    fn get_byte_range_secure(&self, urn: &Urn) -> SecurityResult<Vec<u8>> {
        ...
        if let Some(byte_range) = &urn.byte_range {
            ...
            let end = byte_range.end.map(|e| (e + 1).min(file_len)).unwrap_or(file_len);
...
mod tests {
    ...
    fn test_access_controller_validation() {
        ...
        let invalid_store_id = Hash::from_hex("1111111111111111111111111111111111111111111111111111111111111111").unwrap();
        let invalid_urn = Urn {
