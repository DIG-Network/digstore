...
impl SecureLayer {
    ...
    pub fn write_to_file(&self, path: &Path, urn: &Urn) -> Result<()> {
        ...
        scrambler.scramble(&mut json_bytes).map_err(|e| {
            DigstoreError::internal(format!("Failed to scramble layer data: {}", e))
        })?;
    ...
    pub fn read_from_file(path: &Path, urn: &Urn) -> Result<Self> {
        ...
        let mut scrambler = DataScrambler::from_urn(urn);
        scrambler.unscramble(&mut scrambled_data).map_err(|e| {
            DigstoreError::internal(format!("Failed to unscramble layer data: {}", e))
        })?;
    ...
    pub fn get_file_data(&self, file_path: &Path, urn: &Urn) -> SecurityResult<Vec<u8>> {
        ...
        let file_entry = self
            ...
            .ok_or_else(|| {
                SecurityError::access_denied(format!("File not found: {}", file_path.display()))
            })?;
        ...
        for chunk in file_chunks {
            ...
            let chunk_urn = Urn {
                ...
                byte_range: Some(crate::urn::ByteRange::new(
                    Some(chunk.offset),
                    Some(chunk.offset + chunk.size as u64 - 1),
                )),
            };

            let mut chunk_scrambler = DataScrambler::from_urn(&chunk_urn);
            chunk_scrambler
                .process_at_offset(&mut chunk_data, chunk.offset)
                .map_err(|e| {
                    SecurityError::access_denied(format!("Failed to unscramble chunk: {}", e))
                })?;
    ...
    pub fn scramble_chunk_data(
        ...
        scrambler
            .process_at_offset(&mut chunk.data, chunk.offset)
            .map_err(|e| {
                SecurityError::scrambling_failed(format!("Chunk scrambling failed: {}", e))
            })?;
...
mod tests {
    ...
    fn test_secure_layer_roundtrip() -> Result<()> {
        ...
        assert_eq!(
            restored_layer.layer.files[0].path,
    ...
    fn test_scrambled_data_unreadable_without_urn() -> Result<()> {
        ...
        let wrong_store_id =
            Hash::from_hex("b3f5c8d9e2b1f4a6c9d8e7f2a5b8c1d4e7f0a3b6c9d2e5f8b1c4d7e0a3b6c9d2")
                .unwrap();
        let wrong_urn = Urn {
    ...
    fn test_deterministic_scrambling() -> Result<()> {
        ...
        assert_eq!(
            data1, data2,
            "Deterministic scrambling should produce identical results"
        );
