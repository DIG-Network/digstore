...
impl AdaptiveProcessor {
    ...
    pub fn process_files_adaptive(
        ...
        if let Some(pb) = progress {
            pb.set_length(files.len() as u64);
                            let message = format!("Processing {} files using {:?} strategy", 
                                       files.len(), 
        ...
        let result = match analysis.recommended_strategy {
            ...
            ProcessingStrategy::Hybrid => {
                self.process_hybrid(&files, progress, &analysis)
            }
            ProcessingStrategy::Individual => {
                self.process_individual(&files, progress)
            }
        }?;
        ...
        let metrics = OperationMetrics {
            ...
            mb_per_second: analysis.total_size as f64 / total_time.as_secs_f64() / (1024.0 * 1024.0),
    ...
    fn process_batch_parallel(
        ...
        let batch_result = self.batch_processor.process_files_batch(files.to_vec(), progress)?;
    ...
    fn process_hybrid(
        ...
        if !analysis.small_files.is_empty() {
            ...
            let batch_result = self.batch_processor.process_files_batch(
                analysis.small_files.clone(), 
                progress
            )?;
        ...
        if !analysis.large_files.is_empty() {
            ...
            let streaming_result = self.process_streaming_large(
        ...
        Ok(AdaptiveProcessingResult {
            ...
            performance_metrics: crate::storage::batch::PerformanceSnapshot {
                files_per_second: files.len() as f64 / processing_time.as_secs_f64(),
                mb_per_second: analysis.total_size as f64 / processing_time.as_secs_f64() / (1024.0 * 1024.0),
                chunks_per_second: 0.0,
                processing_time,
            },
            deduplication_stats: None,
...
impl WorkloadAnalyzer {
    ...
    pub fn analyze_files(&self, files: &[PathBuf]) -> Result<WorkloadAnalysis> {
        ...
        let large_file_size_ratio = if total_size > 0 {
            large_files.iter()
                ...
                .map(|m| m.len())
                .sum::<u64>() as f64 / total_size as f64
        } else {
            0.0
        };
        ...
        let estimated_processing_time = self.estimate_processing_time(&workload_type, total_files, total_size);
    ...
    fn estimate_processing_time(&self, workload_type: &WorkloadType, file_count: usize, total_size: u64) -> Duration {
...
impl PerformanceMonitor {
    ...
    pub fn maybe_tune_config(&mut self) {
        ...
        if let Some(best_operation) = self.recent_operations.iter()
    ...
    fn tune_based_on_best_performance(&mut self, best_strategy: &ProcessingStrategy) {
        ...
        match best_strategy {
            ProcessingStrategy::BatchParallel => {
                ...
                self.current_config.batch_size = (self.current_config.batch_size * 110 / 100).min(1000);
            }
            ProcessingStrategy::StreamingLarge => {
                // Adjust memory mapping threshold
                self.current_config.mmap_threshold = (self.current_config.mmap_threshold * 90 / 100).max(1024 * 1024);
            }
            _ => {}
        }
    }
    ...
    pub fn get_performance_summary(&self) -> PerformanceSummary {
        ...
        let total_time: Duration = self.recent_operations.iter().map(|op| op.processing_time).sum();
        let avg_files_per_second = total_files as f64 / total_time.as_secs_f64();
        
        let best_performance = self.recent_operations.iter()
