...
impl ChunkingEngine {
    ...
    pub fn create_file_entry(&self, path: std::path::PathBuf, data: &[u8]) -> Result<FileEntry> {
        ...
        let chunk_refs: Vec<ChunkRef> = chunks.iter().map(|chunk| ChunkRef {
            hash: chunk.hash,
            offset: chunk.offset,
            size: chunk.size,
        }).collect();
...
mod tests {
    ...
    fn test_large_file_chunking_internal() {
        ...
        for i in 0..(3 * 1024 * 1024) { // 3MB
            large_data.push((i % 256) as u8);
        }
        
        let chunks = engine.chunk_data(&large_data).unwrap();
        
        println!("Created {} chunks for {}MB data", chunks.len(), large_data.len() / (1024 * 1024));
