...
impl OptimizedStagingArea {
    ...
    pub fn add_files_bulk(&mut self, files: Vec<(PathBuf, StagedFile)>) -> Result<BulkStagingResult> {
    ...
    pub fn get_stats(&self) -> StagingStats {
        let total_size = self.staged_files.values()
            .map(|f| f.file_entry.size)
            .sum();
        
        let total_chunks = self.staged_files.values()
            .map(|f| f.chunks.len())
            .sum();
    ...
    fn create_persistence_batch(&mut self) -> Result<()> {
        ...
        let batch_files: Vec<_> = self.dirty_files
            .iter()
            .filter_map(|path| {
                self.staged_files.get(path).map(|f| (path.clone(), f.clone()))
            })
            .collect();
...
impl CompactStagedFile {
    pub fn from_staged_file(staged_file: &StagedFile, path_id: u32) -> Self {
        let first_chunk_hash = staged_file.chunks.first()
            .map(|c| c.hash)
