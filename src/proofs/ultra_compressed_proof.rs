...
impl UltraCompressedProof {
    ...
    pub fn from_archive_proof(proof: &ArchiveSizeProof) -> Result<Self> {
        ...
        let packed_header = if proof.verified_layer_count <= 15 {
            ...
            return Err(DigstoreError::internal(
                "Layer count too large for ultra compression",
            ));
        };
        ...
        compressed_integrity
            .copy_from_slice(&proof.integrity_proofs.archive_header_hash.as_bytes()[..16]);
    ...
    pub fn from_compressed_text(text: &str) -> Result<Self> {
        if let Some(data) = text.strip_prefix("85:") {
            ...
            let bytes =
                hex::decode(text).map_err(|_| DigstoreError::internal("Invalid proof format"))?;
...
fn encode_base94(data: &[u8]) -> String {
    ...
    while num > zero {
        ...
        let digit_idx = if digits.is_empty() {
            0
        } else {
            digits[0] as usize
        };
...
fn decode_base94(text: &str) -> Result<Vec<u8>> {
    ...
    let leading_zeros = text
        .chars()
        .take_while(|&ch| ch == BASE94_CHARSET[0] as char)
        .count();
...
impl UltraCompressedProof {
    ...
    pub fn from_ultra_compressed_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        if decompressed.len() < 66 {
            // Minimum: 1 + 32 + 32 + 1 (header + store_id + root_hash + size_varint)
    ...
    pub fn to_archive_proof(&self) -> Result<ArchiveSizeProof> {
        ...
        if version != 1 {
            return Err(DigstoreError::internal(
                "Unsupported ultra-compressed version",
            ));
        }
