...
impl ArchiveSizeProof {
    ...
    fn generate_integrity_proofs(
        ...
        let first_layer_hash = layer_list.first()
            .map(|(hash, _)| *hash)
            .unwrap_or(Hash::zero());
        let last_layer_hash = layer_list.last()
            .map(|(hash, _)| *hash)
            .unwrap_or(Hash::zero());
...
impl CompressedSizeProof {
    ...
    pub fn from_hex_string(hex_string: &str) -> Result<Self> {
        ...
        let compressed = hex::decode(hex_string)
            .map_err(|_| DigstoreError::internal("Invalid hex string"))?;
...
mod tests {
    ...
    fn test_compressed_proof_roundtrip() {
        ...
        assert_eq!(proof.calculated_total_size, recovered_proof.calculated_total_size);
        assert_eq!(proof.verified_layer_count, recovered_proof.verified_layer_count);
    }
