...
pub fn parse_urn(urn_str: &str) -> Result<Urn> {
    if !urn_str.starts_with("urn:dig:chia:") {
        return Err(DigstoreError::invalid_urn(format!(
            "URN must start with 'urn:dig:chia:', got: {}",
            urn_str
        )));
    }
    ...
    let root_hash =
        if let Some(hash_str) = root_hash_str {
            Some(Hash::from_hex(hash_str).map_err(|_| {
                DigstoreError::invalid_urn(format!("Invalid root hash: {}", hash_str))
            })?)
        } else {
            None
        };
