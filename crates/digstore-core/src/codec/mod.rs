...
impl Encoder {
    ...
    /// Pre-size the backing buffer to `cap` bytes. Lets a caller that knows the
    /// exact framed size (e.g. `4 + ciphertext.len() + merkle_len + 32` for a
    /// `ContentResponse`) write the whole response with a single allocation and
    /// no growth-doubling overshoot — the difference between fitting a near-cap
    /// resource in the module memory ceiling and OOMing on the Vec-growth slack.
