...
fn chunking_benchmark(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("chunk_data", size), &data, |b, data| {
            b.iter(|| engine.chunk_data(black_box(data)).unwrap())
        });
    }
...
fn hashing_benchmark(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("sha256", size), &data, |b, data| {
            b.iter(|| digstore_min::core::hash::sha256(black_box(data)))
        });
    }
...
fn merkle_tree_benchmark(c: &mut Criterion) {
    ...
    for count in hash_counts {
        ...
        group.bench_with_input(
            ...
            |b, hashes| {
                b.iter(|| {
                    digstore_min::proofs::merkle::MerkleTree::from_hashes(black_box(hashes))
                        .unwrap()
                })
            },
        );
        ...
        if count > 1 {
            ...
            group.bench_with_input(
                ...
                &(tree, leaf_index),
                |b, (tree, leaf_index)| {
                    b.iter(|| tree.generate_proof(black_box(*leaf_index)).unwrap())
                },
            );
        }
    }
...
fn file_operations_benchmark(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            ...
            &file_path,
            |b, file_path| b.iter(|| engine.chunk_file(black_box(file_path)).unwrap()),
        );
    }
