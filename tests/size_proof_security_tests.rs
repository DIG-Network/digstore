...
impl SecurityTestSetup {
    fn new() -> anyhow::Result<Self> {
        ...
        fs::write(project_path2.join("large.txt"), "Large content ".repeat(1000))?;
        fs::create_dir_all(project_path2.join("subdir"))?;
        fs::write(project_path2.join("subdir").join("nested.txt"), "Nested content ".repeat(500))?;
        
        // Initialize both repositories
        for (path, name) in [(&project_path1, "Security Test 1"), (&project_path2, "Security Test 2")] {
...
fn test_store_id_binding() -> anyhow::Result<()> {
    ...
    Command::cargo_bin("digstore")?
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Verification failed").or(predicate::str::contains("mismatch")));
...
fn test_proof_generation_parameter_validation() -> anyhow::Result<()> {
    ...
    Command::cargo_bin("digstore")?
        ...
        .assert()
        .failure()
        .stderr(predicate::str::contains("Size mismatch").or(predicate::str::contains("does not match")));
...
fn test_proof_tampering_resistance() -> anyhow::Result<()> {
    ...
    let tampering_tests = vec![
        ...
        ("extend", {
            format!("{}DEADBEEF", original_proof)
        }),
        ("replace_section", {
            let mut tampered = original_proof.clone();
...
fn test_deterministic_proof_generation() -> anyhow::Result<()> {
    ...
    for (i, proof) in proofs.iter().enumerate().skip(1) {
        assert_eq!(proof, first_proof, 
                  "Proof {} differs from first proof - not deterministic", i);
    }
    
    println!("✓ All {} proof generations produced identical results", proofs.len());
...
fn test_proof_uniqueness() -> anyhow::Result<()> {
    ...
    if proof1.len() == proof2.len() {
        let different_chars = proof1.chars()
            .zip(proof2.chars())
            .filter(|(a, b)| a != b)
            .count();
        
        assert!(different_chars > 10, 
               "Proofs are too similar - only {} characters differ", different_chars);
    }
...
fn test_cryptographic_integrity() -> anyhow::Result<()> {
    ...
    assert!(output_str.contains("Merkle tree"), "Should mention merkle tree construction");
    assert!(output_str.contains("Layer count"), "Should show layer count");
    assert!(output_str.contains("Total size"), "Should show total size calculation");
    
    // Extract the actual proof (last line should be hex)
    let proof_hex = output_str.lines()
        .last()
    ...
    assert!(proof_hex.chars().all(|c| c.is_ascii_hexdigit()), "Proof should be hex");
