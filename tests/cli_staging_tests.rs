...
fn test_staged_command_with_no_files() -> Result<()> {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(
                e.to_string().contains("Not in a repository")
...
fn test_staging_clear_validation() -> Result<()> {
    ...
    assert_eq!(
        size_after, 88,
        "Staging file should be reset to header-only"
    );
...
fn test_binary_staging_vs_old_json_format() -> Result<()> {
    ...
    for i in 1..=20 {
        fs::write(
            project_path.join(format!("efficiency{}.txt", i)),
            format!("content {}", i),
        )?;
    }
    ...
    assert!(
        binary_staging_size < 10000,
        "Binary staging should be efficient: {} bytes for 20 files",
        binary_staging_size
    );
    ...
    assert_eq!(
        store.status().staged_files.len(),
        20,
        "Should handle 20 files correctly"
    );
...
fn test_regression_prevention_checklist() -> Result<()> {
    ...
    // Create test file
    fs::write(
        project_path.join("regression.txt"),
    ...
    assert_eq!(
        all_files.len(),
        1,
        "Iterator fix: get_all_staged_files should work"
    );

    // Test 2: Memory map refresh - file should be retrievable
    let retrieved = store
        .staging
        .get_staged_file(&Path::new("regression.txt"))?;
    assert!(
        retrieved.is_some(),
        "Memory map refresh: file should be retrievable"
    );
    ...
    assert_eq!(
        status.staged_files.len(),
        1,
        "Status mutation fix: should work"
    );
    ...
    assert_eq!(
        ...
        0,
        "Commit clearing: staging should be empty"
    );
