...
fn test_add_status_commit_workflow_integration() -> Result<()> {
    ...
    assert!(
        status.total_staged_size > 0,
        "Should have non-zero staged size"
    );

    // Step 3: Commit should work
    let commit_id = test.store.commit("Test commit")?;
    assert!(
        !commit_id.to_hex().is_empty(),
        "Should return valid commit ID"
    );

    // Step 4: Status should show no staged files after commit
    let status_after = test.store.status();
    assert_eq!(
        status_after.staged_files.len(),
        0,
        "Staging should be cleared after commit"
    );
    assert_eq!(
        status_after.total_staged_size, 0,
        "Staged size should be zero after commit"
    );
...
fn test_staging_persistence_across_cli_commands() -> Result<()> {
    ...
    // Simulate second CLI command: check status
    {
        ...
        assert_eq!(
            ...
            2,
            "Files should persist across CLI commands"
        );
...
fn test_commit_properly_clears_staging_file() -> Result<()> {
    ...
    assert!(
        staging_size_before > 88,
        "Staging file should contain data (more than just header)"
    );
    ...
    assert_eq!(
        status_after.staged_files.len(),
        0,
        "Staging should be cleared in memory"
    );

    // Verify staging file is physically cleared
    let staging_size_after = fs::metadata(&staging_path)?.len();
    assert_eq!(
        staging_size_after, 88,
        "Staging file should be reset to header-only size"
    );
...
fn test_multiple_add_commit_cycles() -> Result<()> {
    ...
    for i in 1..=6 {
        fs::write(
            project_path.join(format!("cycle{}.txt", i)),
            format!("content {}", i),
        )?;
    }
    ...
    assert_eq!(
        status1.staged_files.len(),
        2,
        "Cycle 1: Should have 2 staged files"
    );
    ...
    assert_eq!(
        status1_after.staged_files.len(),
        0,
        "Cycle 1: Staging should be cleared"
    );
    ...
    assert_eq!(
        status2.staged_files.len(),
        3,
        "Cycle 2: Should have 3 staged files"
    );
    ...
    assert_eq!(
        status2_after.staged_files.len(),
        0,
        "Cycle 2: Staging should be cleared"
    );
    ...
    assert_eq!(
        status3.staged_files.len(),
        1,
        "Cycle 3: Should have 1 staged file"
    );
    ...
    assert_eq!(
        status3_after.staged_files.len(),
        0,
        "Cycle 3: Staging should be cleared"
    );
...
fn test_staging_file_corruption_prevention() -> Result<()> {
    ...
    for i in 1..=10 {
        fs::write(
            project_path.join(format!("batch{}.txt", i)),
    ...
    assert_eq!(
        status2.staged_files.len(),
        5,
        "Batch 2: Should have 5 total files"
    );
    ...
    for i in 1..=5 {
        ...
        assert!(
            store.is_file_staged(Path::new(&file_path)),
    ...
    assert_eq!(
        status_after.staged_files.len(),
        0,
        "All files should be cleared after commit"
    );
...
fn test_staging_system_memory_efficiency() -> Result<()> {
    ...
    assert!(
        staging_size < 10000,
        "Staging file should be compact (< 10KB for 3 small files)"
    );
    assert!(
        staging_size > 88,
        "Staging file should contain more than just header"
    );
