...
fn test_complete_system_demo() -> Result<()> {
    ...
    // Create demo files
    std::fs::write(temp_dir.path().join("README.md"), b"# Demo Project\n\nThis is a demonstration of Digstore Min.\n")?;
    std::fs::write(temp_dir.path().join("config.json"), b"{\n  \"name\": \"demo\",\n  \"version\": \"1.0.0\"\n}\n")?;
    std::fs::write(temp_dir.path().join("data.txt"), b"Important data that needs to be versioned and verified.\n")?;
    ...
    assert_eq!(readme_content, b"# Demo Project\n\nThis is a demonstration of Digstore Min.\n");
    
    let config_content = store.get_file(Path::new("config.json"))?;
    println!("   ✓ Retrieved config.json ({} bytes)", config_content.len());
    ...
    println!("   ✓ config.json still accessible ({} bytes)", config_still_there.len());
    ...
    println!("   ✓ Retrieved README.md from first commit ({} bytes)", old_readme.len());
    assert_eq!(old_readme, readme_content);
    
    let new_readme = store.get_file_at(Path::new("README.md"), Some(commit2))?;
    println!("   ✓ Retrieved README.md from second commit ({} bytes)", new_readme.len());
    assert_eq!(new_readme, updated_readme);
    
    // Show final status
    println!("\n8. Final repository status:");
    println!("   ✓ Current root: {}", store.current_root().unwrap().to_hex());
    println!("   ✓ Store location: {}", store.global_path().display());
    println!("   ✓ Staging area: {} files", store.status().staged_files.len());
    
    // Test chunking on a larger file
    println!("\n9. Testing content-defined chunking...");
    let mut large_content = String::new();
    for i in 0..1000 {
        large_content.push_str(&format!("Line {} of large file content for chunking demonstration.\n", i));
    }
    
    std::fs::write(temp_dir.path().join("large_file.txt"), large_content.as_bytes())?;
    ...
    println!("   ✓ Retrieved large file ({} bytes)", retrieved_large.len());
