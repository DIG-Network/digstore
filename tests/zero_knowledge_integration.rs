    ...
    for urn in invalid_urns {
        ...
        match result {
            Ok(parsed) => {
                ...
                println!(
                    "URN '{}' parsed successfully: store_id={}",
                    urn,
                    parsed.store_id.to_hex()
                );
            },
            Err(_) => {
...
fn test_urn_transformation_determinism() {
    use digstore_min::crypto::{transform_urn, PublicKey};

    let public_key =
        PublicKey::from_hex("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")
            .unwrap();
    let urn =
        "urn:dig:chia:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890/test.txt";
    ...
    assert_eq!(
        ...
        "URN transformation should be deterministic"
    );
    assert_eq!(
        transformed1, transformed3,
        "URN transformation should be deterministic"
    );
    ...
    assert_eq!(
        transformed1.len(),
        64,
        "Transformed address should be 64 hex characters"
    );
    assert!(
        transformed1.chars().all(|c| c.is_ascii_hexdigit()),
        "Should be valid hex"
    );
}
...
fn test_storage_address_derivation() {
    use digstore_min::crypto::{derive_storage_address, PublicKey};
    ...
    assert_eq!(
        addr1, addr2,
        "Storage address derivation should be deterministic"
    );
    assert_eq!(
        addr1.len(),
        64,
        "Storage address should be 64 hex characters"
    );
    assert!(
