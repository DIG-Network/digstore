...
use digstore_min::{
    core::types::*,
    storage::{BatchProcessor, OptimizedFileScanner, Store},
};
use std::fs;
use std::io::Write;
...
fn test_many_small_files_batch_processing() -> anyhow::Result<()> {
    ...
    for i in 0..file_count {
        ...
        let content = format!(
            "This is small file number {} with some content to make it realistic",
            i
        );
    ...
    assert!(
        ...
        "Should process >50 files/second, got {:.1}",
        files_per_second
    );
...
fn test_mixed_workload_performance() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Mixed workload should complete in <2 minutes (with security overhead)"
    );
...
fn test_directory_batch_processing() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should process >20 files/second for directory operations"
    );
    assert!(
        total_time.as_secs() < 20,
        "Directory processing should complete in <20 seconds"
    );
...
fn test_memory_efficiency() -> anyhow::Result<()> {
    ...
    println!(
        ...
        processing_time / file_count
    );

    // Should be efficient
    assert!(
        processing_time.as_secs() < 10,
        "Should process {} files in <10 seconds",
        file_count
    );
    ...
    assert!(
        retrieval_time.as_secs() < 30,
        "File retrieval should be reasonably fast (with security overhead)"
    );
...
fn test_batch_processor_performance() -> anyhow::Result<()> {
    ...
    println!(
        "   MB/second: {:.1}",
        result.performance_metrics.mb_per_second
    );
    println!(
        "   Deduplication ratio: {:.1}%",
        result.deduplication_stats.deduplication_ratio * 100.0
    );

    // Performance requirements
    assert!(
        files_per_second > 100.0,
        "Batch processor should handle >100 files/second"
    );
    assert!(
        processing_time.as_secs() < 5,
        "Should process {} files in <5 seconds",
        file_count
    );
...
fn test_file_scanning_performance() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Should scan 500 files in <1 second"
    );
...
fn test_adaptive_processing_selection() -> anyhow::Result<()> {
    ...
    assert!(
        ...
        "Few files should process very quickly"
    );
    ...
    assert!(
        batch_files_per_second > 50.0,
        "Batch processing should be >50 files/second"
    );
