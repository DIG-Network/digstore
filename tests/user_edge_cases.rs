...
fn test_special_characters_in_filenames() {
    ...
    // User checks staging
    let staged_output = Command::cargo_bin("digstore").unwrap()
        .current_dir(project_path)
        .arg("staged")
        .assert()
        .success()
...
fn test_file_permission_scenarios() {
    ...
    fs::write(project_path.join("binary.bin"), vec![0u8, 1u8, 255u8, 128u8]).unwrap();
...
fn test_network_and_sharing_scenarios() {
    ...
    fs::write(project_path.join("shared_doc.md"), "# Shared Document\n\nThis is shared content.").unwrap();
    fs::write(project_path.join("data.json"), r#"{"shared": true, "version": "1.0"}"#).unwrap();
...
fn test_performance_stress_scenarios() {
    ...
    fs::write(project_path.join("medium.txt"), "medium content ".repeat(100)).unwrap();
...
fn test_cross_platform_compatibility() {
    ...
    fs::write(project_path.join("windows.txt"), "Line 1\r\nLine 2\r\nLine 3").unwrap();
...
fn test_real_world_project_simulation() {
    ...
    fs::write(project_path.join("src/lib.rs"), "pub mod components;\npub mod utils;").unwrap();
    fs::write(project_path.join("src/components/button.rs"), "// Button component").unwrap();
    fs::write(project_path.join("src/utils/helpers.rs"), "// Helper functions").unwrap();

    // Test files
    fs::write(project_path.join("tests/unit/test_main.rs"), "// Unit tests").unwrap();
    fs::write(project_path.join("tests/integration/test_api.rs"), "// Integration tests").unwrap();

    // Documentation
    fs::write(project_path.join("README.md"), "# Real World Project\n\nA test project.").unwrap();
    ...
    // Assets
    fs::write(project_path.join("assets/config.json"), "{\"app\": \"config\"}").unwrap();
    fs::write(project_path.join("assets/styles.css"), "body { margin: 0; }").unwrap();
...
fn test_comprehensive_help_system() {
    ...
    let commands = [
        "init", "add", "commit", "status", "get", "cat", 
        "staged", "config", "store", "layer", "proof"
    ];
