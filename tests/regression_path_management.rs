...
fn test_path_auto_fix_safety() {
    ...
    if output.status.success() {
        ...
        assert!(
            stderr.contains("PATH") || stderr.contains("permission") || stderr.contains("access"),
...
fn test_environment_variable_safety() -> anyhow::Result<()> {
    ...
    assert_eq!(
        reconstructed_path, original_path,
        "PATH reconstruction should be accurate"
    );
    ...
    assert!(
        new_path.starts_with(test_dir),
        "New PATH should start with test directory"
    );
    assert!(
        new_path.contains(&original_path),
        "New PATH should contain original PATH"
    );
    ...
    assert_eq!(
        ...
        "Filtering non-existent directory should not change PATH"
    );
...
fn test_version_detection_robustness() -> anyhow::Result<()> {
    ...
    match active_version {
        ...
        Err(e) => {
            println!(
                "Version detection failed (acceptable in test environment): {}",
                e
            );
        },
    }
...
fn test_multiple_version_isolation() -> anyhow::Result<()> {
    ...
    if !successful_installs.is_empty() {
        println!(
            "Successfully installed {} test versions",
    ...
    match update_check {
        Ok(info) => {
            ...
            if info.update_available {
                ...
                assert_ne!(
                    info.current_version, info.latest_version,
                    "Available update should have different versions"
                );
            } else {
        ...
        Err(e) => {
            println!(
                "Update check failed (expected in offline test environment): {}",
                e
            );
        },
    }
