...
fn get_global_store_path(store_id: &digstore_min::core::types::StoreId) -> anyhow::Result<std::path::PathBuf> {
            ...
            if entry.file_type()?.is_dir() {
                if let Ok(metadata) = entry.metadata() {
                    if let Ok(created) = metadata.created() {
                        let age = created.elapsed().unwrap_or(std::time::Duration::from_secs(0));
