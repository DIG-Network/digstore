...
fn test_archive_integration_commands_work() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Info command should work with archive format"
    );
    ...
    assert!(
        result.is_ok(),
        "Log command should work with archive format"
    );
    ...
    assert!(
        result.is_ok(),
        "Root command should work with archive format"
    );
    ...
    assert!(
        result.is_ok(),
        "Stats command should work with archive format"
    );
    ...
    assert!(
        result.is_ok(),
        "Size command should work with archive format"
    );
...
fn test_store_id_generation_robustness() -> Result<()> {
    ...
    for _ in 0..100 {
        ...
        assert_ne!(
            store_id,
            digstore_min::core::types::Hash::zero(),
...
fn test_layer_file_size_calculation_accuracy() -> Result<()> {
    ...
    for (layer_hash, entry) in layers {
        ...
        assert!(
            ...
            "Layer size should be reasonable (<100MB for test)"
        );

        // Layer should have some content
        assert!(
            layer.files.len() > 0 || layer.chunks.len() > 0,
...
fn test_archive_stats_calculation() -> Result<()> {
    ...
    assert!(
        stats.data_size <= stats.total_size,
        "Data size should not exceed total size"
    );
    assert!(
    ...
    assert!(
        stats.fragmentation >= 0.0 && stats.fragmentation <= 1.0,
...
fn test_cli_commands_handle_missing_data_gracefully() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Info command should handle empty repository"
    );
    ...
    assert!(
        result.is_ok(),
        "Log command should handle no commits gracefully"
    );
    ...
    assert!(
        result.is_ok(),
        "Root command should handle no commits gracefully"
    );
    ...
    assert!(
        result.is_ok(),
        "Stats command should handle empty repository"
    );
...
fn test_archive_layer_listing_consistency() -> Result<()> {
    ...
    let has_layer_zero = layers
        .iter()
        .any(|(hash, _)| *hash == digstore_min::core::types::Hash::zero());
    ...
    for (layer_hash, _entry) in layers {
        if layer_hash == digstore_min::core::types::Hash::zero() {
            ...
            assert!(
                result.is_ok(),
                "Layer 0 should be readable as metadata: {}",
                layer_hash
            );
        } else {
...
fn test_staging_path_calculation() -> Result<()> {
    ...
    assert_eq!(
        actual_staging_path, &expected_staging_path,
...
fn test_unwrap_safety_in_critical_paths() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Should return error for nonexistent file, not panic"
    );
