...
fn test_core_staging_commands() -> Result<()> {
    ...
    assert!(stdout.contains("No changes staged"), "Should show no staged changes after commit");
    ...
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("No files staged"), "Should show no staged files");
...
fn test_data_access_commands() -> Result<()> {
    ...
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("Hello World"), "Cat should show file content");
    
    // Test log command
    let output = repo.run_command(&["log"])?;
    assert!(output.status.success(), "Log command should succeed");
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("Initial test commit"), "Log should show commit message");
...
fn test_analysis_commands() -> Result<()> {
    ...
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("Repository Statistics"), "Stats should show statistics");
...
fn test_advanced_commands() -> Result<()> {
    ...
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("Proof generated"), "Prove should generate proof");
...
fn test_completion_command() -> Result<()> {
    ...
    for shell in &shells {
        ...
        assert!(output.status.success(), "Completion for {} should succeed", shell);
        let stdout = String::from_utf8(output.stdout)?;
        assert!(!stdout.is_empty(), "Completion should generate script for {}", shell);
    }
...
fn test_end_to_end_workflow() -> Result<()> {
    ...
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("No changes staged"), "Should show clean state");
...
fn test_add_all_workflow() -> Result<()> {
    ...
    for i in 1..=5 {
        fs::write(project_path.join(format!("file{}.txt", i)), format!("Content {}", i))?;
    }
    ...
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("files added to staging"), "Should show files added");
    ...
    for i in 1..=5 {
        ...
        assert!(output.status.success(), "Should be able to get file{}.txt", i);
    }
...
fn test_root_tracking_persistence() -> Result<()> {
    ...
    assert!(commit_stdout.contains("Commit ID:"), "Should show commit ID");
    ...
    assert!(!status_stdout.contains("none (no commits yet)"), "Should show actual commit");
    ...
    assert!(log_stdout.contains("Persistence test"), "Log should show commit message");
...
fn test_command_error_handling() -> Result<()> {
    ...
    assert!(stderr.contains("Not in a repository") || stderr.contains("No repository found"), 
    ...
    assert!(!output.status.success(), "Get non-existent file should fail");
    
    // Test cat non-existent file
    let output = cmd_helper(&["cat", "nonexistent.txt"])?;
    assert!(!output.status.success(), "Cat non-existent file should fail");
...
fn test_staging_system_regression_prevention() -> Result<()> {
    ...
    // Create test files
    for i in 1..=10 {
        fs::write(project_path.join(format!("regression{}.txt", i)), format!("Content {}", i))?;
    }
    ...
    let output = cmd_helper(&["--yes", "add", "regression1.txt", "regression2.txt", "regression3.txt"])?;
    ...
    assert!(output.status.success(), "Staged command should succeed");
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("3") || stdout.contains("regression"), "Should show staged files");
    ...
    let output = cmd_helper(&["staged"])?;
    assert!(output.status.success(), "Staged after commit should succeed");
    let stdout = String::from_utf8(output.stdout)?;
    assert!(stdout.contains("No files staged"), "Staging should be cleared after commit");
    
    // Test files are accessible
    let output = cmd_helper(&["get", "regression1.txt"])?;
    assert!(output.status.success(), "Should be able to get committed file");
...
fn test_command_consistency() -> Result<()> {
    ...
    assert!(!status_stdout.contains("none (no commits yet)"), "Status should show commit");
    assert!(log_stdout.contains("Initial test commit"), "Log should show commit");
