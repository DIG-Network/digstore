...
fn test_version_manager_creation() -> anyhow::Result<()> {
    ...
    match vm_result {
        ...
        Err(_) => {
            println!("Skipping version manager test - cannot create VersionManager (expected in CI)");
        }
    }
...
fn test_binary_installation_simulation() -> anyhow::Result<()> {
    ...
    match install_result {
        ...
        Err(e) => {
            println!("Binary installation simulation failed (expected in test environment): {}", e);
        }
    }
