...
fn test_smart_staging_skips_unchanged_files() -> Result<()> {
    ...
    assert_eq!(
        staged_files.len(),
        0,
        "Unchanged files should not be staged"
    );
...
fn test_has_file_changed_method() -> Result<()> {
    ...
    assert!(
        !test.store.has_file_changed(Path::new("test.txt"))?,
    ...
    test.create_file("new.txt", "new content")?;
    assert!(
        test.store.has_file_changed(Path::new("new.txt"))?,
        "New file should be detected as changed"
    );
...
fn test_stage_diff_detects_all_change_types() -> Result<()> {
    ...
    assert!(
        staged_files.len() >= 2,
        "Should have staged the modified and new files"
    );
...
fn test_stage_diff_specific_file_filter() -> Result<()> {
    ...
    assert_eq!(
        staged_files.len(),
        2,
        "Both modified files should be staged"
    );

    // Verify the files are the ones we expect
    let staged_names: Vec<_> = staged_files
        .iter()
...
fn test_smart_staging_performance_benefit() -> Result<()> {
    ...
    assert!(
        ...
        "Smart staging should be fast for unchanged files"
    );
...
fn test_mixed_changed_and_unchanged_files() -> Result<()> {
    ...
    assert_eq!(
        staged_files.len(),
        5,
        "Only modified files should be staged"
    );
    ...
    for i in 0..5 {
        ...
        assert!(
            staged_names.contains(&expected_name),
...
fn test_stage_diff_command_integration() -> Result<()> {
    ...
    assert_eq!(
        staged_files.len(),
        2,
        "Should have staged 2 files (original and new)"
    );

    // Test file change detection
    assert!(
        test.store.has_file_changed(Path::new("original.txt"))?,
        "Modified file should be detected as changed"
    );
...
fn test_stage_diff_size_calculations() -> Result<()> {
    ...
    assert!(
        result.is_ok(),
        "Stage diff should calculate size changes correctly"
    );
...
fn test_smart_staging_with_empty_repository() -> Result<()> {
    ...
    assert!(
        test.store.has_file_changed(Path::new("file.txt"))?,
    ...
    assert_eq!(
        staged_files.len(),
        1,
        "File should be staged in empty repository"
    );
