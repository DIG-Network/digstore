...
use digstore_min::proofs::size_proof::{ArchiveSizeProof, verify_archive_size_proof, verify_compressed_hex_proof};
...
impl TestArchive {
    fn new() -> anyhow::Result<Self> {
        ...
        // Create test content
        fs::write(project_path.join("test1.txt"), "Test content for proof generation")?;
        fs::write(project_path.join("test2.txt"), "More test content with different size")?;
        fs::create_dir_all(project_path.join("subdir"))?;
        fs::write(project_path.join("subdir").join("nested.txt"), "Nested file content")?;
        ...
        let archive_path = format!("{}/.dig/{}.dig", 
...
fn test_proof_generation_basic() -> anyhow::Result<()> {
    ...
    assert!(proof_hex.chars().all(|c| c.is_ascii_hexdigit()), "Proof should be hex-encoded");
...
fn test_proof_generation_with_output_file() -> anyhow::Result<()> {
    ...
    assert!(!proof_content.trim().is_empty(), "Proof file should not be empty");
    assert!(proof_content.chars().all(|c| c.is_ascii_hexdigit() || c.is_whitespace()), 
...
fn test_proof_verification_success() -> anyhow::Result<()> {
    ...
    Command::cargo_bin("digstore")?
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("✓ Archive size proof verified successfully"));
...
mod error_cases {
    ...
    fn test_nonexistent_archive() {
        Command::cargo_bin("digstore").unwrap()
            ...
            .assert()
            .failure()
            .stderr(predicate::str::contains("Archive not found").or(predicate::str::contains("No such file")));
    }
    ...
    fn test_invalid_proof_verification() {
        Command::cargo_bin("digstore").unwrap()
            ...
            .assert()
            .failure()
            .stderr(predicate::str::contains("Invalid hex").or(predicate::str::contains("Failed to decode")));
    }
    ...
    fn test_corrupted_proof_verification() -> anyhow::Result<()> {
        ...
        Command::cargo_bin("digstore")?
            ...
            .assert()
            .failure()
            .stderr(predicate::str::contains("Verification failed").or(predicate::str::contains("Invalid proof")));
...
mod json_output {
    ...
    fn test_proof_verification_json_output() -> anyhow::Result<()> {
        ...
        assert_eq!(json_output["archive_size"].as_u64().unwrap(), test_archive.archive_size);
...
mod performance {
    ...
    fn test_proof_generation_speed() -> anyhow::Result<()> {
        ...
        assert!(duration.as_secs() < 5, 
               "Proof generation took too long: {:?}", duration);
    ...
    fn test_proof_verification_speed() -> anyhow::Result<()> {
        ...
        assert!(duration.as_secs() < 1, 
               "Proof verification took too long: {:?}", duration);
    ...
    fn test_proof_compression_ratio() -> anyhow::Result<()> {
        ...
        assert!(proof_only.len() < 1000, 
               "Proof not sufficiently compressed: {} chars", proof_only.len());
        
        // But should still contain substantial data (> 100 characters)
        assert!(proof_only.len() > 100, 
               "Proof too small, might be missing data: {} chars", proof_only.len());
