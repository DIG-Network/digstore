...
fn test_layers_command_with_multiple_commits() -> Result<()> {
    ...
    assert!(archive_layers.len() >= 4, "Should have at least 4 layers (Layer 0 + 3 commits)");
    
    // Verify specific layers exist
    let layer_zero_hash = digstore_min::core::types::Hash::zero();
    assert!(store.archive.has_layer(&layer_zero_hash), "Should have Layer 0");
    ...
    assert_eq!(loaded_layer2.files.len(), 2, "Layer 2 should have 2 files (cumulative)");
    assert_eq!(loaded_layer3.files.len(), 4, "Layer 3 should have 4 files (cumulative)");
    
    // Test that layers command can list all layers
    let listed_layers: Vec<_> = archive_layers.into_iter()
        .filter(|(hash, _)| *hash != layer_zero_hash) // Exclude Layer 0
        .collect();
    
    assert_eq!(listed_layers.len(), 3, "Should list exactly 3 commit layers");
    
    // Verify layer hashes match
    let layer_hashes: Vec<_> = listed_layers.iter().map(|(hash, _)| *hash).collect();
    assert!(layer_hashes.contains(&layer1), "Should contain layer 1 hash");
    assert!(layer_hashes.contains(&layer2), "Should contain layer 2 hash");  
    assert!(layer_hashes.contains(&layer3), "Should contain layer 3 hash");
...
fn test_layers_command_with_empty_repository() -> Result<()> {
    ...
    let archive_layers = store.archive.list_layers();
    assert_eq!(archive_layers.len(), 1, "Empty repository should have only Layer 0");
    
    let layer_zero_hash = digstore_min::core::types::Hash::zero();
    assert!(store.archive.has_layer(&layer_zero_hash), "Should have Layer 0");
    
    // Current root should be None for empty repository
    assert_eq!(store.current_root(), None, "Empty repository should have no current root");
...
fn test_layers_command_archive_consistency() -> Result<()> {
    ...
    assert!(store.archive.has_layer(&commit_hash), "Archive should contain the layer");
    ...
    assert!(layer.header.timestamp > 0, "Layer should have valid timestamp");
