...
fn test_verify_command_proof_validation() -> Result<()> {
    ...
    assert!(matches!(
        proof.target,
        digstore_min::proofs::proof::ProofTarget::File { .. }
    ));
...
fn test_inspect_command_layer_access() -> Result<()> {
    ...
    assert!(file_paths
        .iter()
        .any(|p| p.to_string_lossy().contains("test1.txt")));
    assert!(file_paths
        .iter()
        .any(|p| p.to_string_lossy().contains("test2.txt")));
...
fn test_root_tracking_consistency() -> Result<()> {
    ...
    let latest_root_str = root_history
        .last()
